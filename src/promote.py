...
import config
from serving.db import get_current_batch, get_engine, get_labeled_predictions, get_performance_window, get_state, init_db, log_audit_event, set_state
...
def bootstrap_promote(engine, client, challenger_version):
    ...
    client.set_registered_model_alias(config.MODEL_NAME, "production", str(challenger_version))
...
def check_rollback(engine, client):
    ...
    client.set_registered_model_alias(config.MODEL_NAME, "production", str(previous_version))
...
def main():
    ...
    if args.mode == "check-rollback":
        with logfire.span("check_rollback"):
            rolled_back = check_rollback(engine, client)
            logfire.info("rollback_check_result", rolled_back=rolled_back)
    ...
    with logfire.span("evaluate_and_promote", challenger_version=args.challenger_version):
        promoted = evaluate_and_promote(engine, client, args.challenger_version)
        logfire.info("promotion_decision", challenger_version=args.challenger_version, promoted=promoted)
